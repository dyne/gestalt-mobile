...
async function sandbox(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'skill-profile-'));
  roots.push(root);
  return root;
}
const profile = () =>
  createSkillProfile({
    name: 'Work',
    skills: [{ name: 'Alpha', path: '/skills/alpha/SKILL.md', enabled: true }],
  });
...
describe('FilesystemSkillProfileStore', () => {
  it('returns missing global profiles, creates their private directory, and lists names deterministically', async () => {
    ...
    expect(
      (await (await import('node:fs/promises')).stat(join(home, '.gestalt', 'skill-profiles')))
        .mode & 0o777,
    ).toBe(0o700);
  });
  ...
  it('atomically overwrites a global profile and leaves no temporary file', async () => {
    ...
    expect(
      (
        await (await import('node:fs/promises')).readdir(join(home, '.gestalt', 'skill-profiles'))
  ...
  it('reads only the exact workspace-root gestalt-skills.yml file', async () => {
    ...
    await mkdir(nested);
    await writeFile(
      join(workspace, 'gestalt-skills.yml'),
      'version: 1\nname: project\nskills: []\n',
    );
    await writeFile(join(nested, 'gestalt-skills.yml'), 'version: 1\nname: nested\nskills: []\n');
    await expect(
      new FilesystemSkillProfileStore(await sandbox()).readWorkspaceDefault(nested),
    ).resolves.toEqual({ version: 1, name: 'nested', skills: [] });
    await expect(
      new FilesystemSkillProfileStore(await sandbox()).readWorkspaceDefault(workspace),
    ).resolves.toEqual({ version: 1, name: 'project', skills: [] });
  });
  ...
  it('rejects profile symlinks and invalid YAML', async () => {
    ...
    await expect(
      new FilesystemSkillProfileStore(home).readGlobalProfile('work'),
    ).rejects.toBeInstanceOf(SkillProfileError);
  ...
  it('does not follow profile or root symlinks when deleting', async () => {
    ...
    await expect(
      new FilesystemSkillProfileStore(home).deleteGlobalProfile('work'),
    ).rejects.toBeInstanceOf(SkillProfileError);
    await expect(
      (await import('node:fs/promises')).readFile(join(outside, 'target.yml'), 'utf8'),
    ...
    await expect(
      new FilesystemSkillProfileStore(linkedHome).deleteGlobalProfile('target'),
    ).rejects.toBeInstanceOf(SkillProfileError);
    await expect(
      (await import('node:fs/promises')).readFile(join(outside, 'target.yml'), 'utf8'),
    ).resolves.toContain('name: target');
