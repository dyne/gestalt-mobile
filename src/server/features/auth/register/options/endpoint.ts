...
export function registerRegistrationOptions(app: FastifyInstance, deps: { repository: AuthorizationRepository; clock: Clock; random: RandomBytes; webauthn: WebAuthnCeremonyService; relyingParty: { publicOrigin: string; rpId: string; rpName: string } }): void {
  app.post('/api/auth/register/options', async (request, reply) => {
    ...
    if (ownerHandle && ownerHandle.length !== 32) throw new Error('AUTHORIZATION_RANDOMNESS_INVALID');
    const initialized = owner ?? deps.repository.initializeOwner(ownerHandle!);
    const correlation = deps.random.bytes(32); const challenge = deps.random.bytes(32);
    if (correlation.length !== 32 || challenge.length !== 32) throw new Error('AUTHORIZATION_RANDOMNESS_INVALID');
    const token = passkeyCeremonyId(Buffer.from(correlation).toString('base64url'));
    const options = optionsResponseSchema.parse({ options: await deps.webauthn.registrationOptions({ challenge, rpId: deps.relyingParty.rpId, userVerification: 'required', rpName: deps.relyingParty.rpName, userHandle: initialized.userHandle, excludeCredentialIds: devices.map((device) => device.credentialId) }) });
    deps.repository.saveCeremony(token, { purpose: 'registration', challenge, expectedOrigin: deps.relyingParty.publicOrigin, rpId: deps.relyingParty.rpId, expiresAt: new Date(now.getTime() + lifetimeMs).toISOString(), ...(ticket ? { enrollmentTicket: ticket } : {}) });
    reply.setCookie('gestalt_mobile_registration', token, { httpOnly: true, sameSite: 'strict', path: '/', secure: deps.relyingParty.publicOrigin.startsWith('https://'), maxAge: 600 });
