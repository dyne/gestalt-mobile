...
const device = (
  id: string,
  credential: string,
  type: AuthorizedDevice['deviceType'] = 'singleDevice',
) => ({
...
describe('DeviceNickname', () => {
  ...
  it('refuses empty and overlong nicknames', () => {
    expect(() => deviceNickname('   ')).toThrow(
      new AuthorizationDomainError('DEVICE_NICKNAME_INVALID'),
    );
    expect(() => deviceNickname(' \u00a0 ')).toThrow(
      new AuthorizationDomainError('DEVICE_NICKNAME_INVALID'),
    );
    expect(() => deviceNickname('a'.repeat(65))).toThrow(
      new AuthorizationDomainError('DEVICE_NICKNAME_INVALID'),
    );
  });
});
...
describe('LocalOwnerAuthorization', () => {
  ...
  it('starts with a first device and rejects duplicate credentials', () => {
    const authorization = LocalOwnerAuthorization.bootstrap(
      owner,
      device('device-1', 'credential-1'),
    );
  ...
  it('rejects duplicate authorized-device identities', () => {
    ...
    expect(() =>
      LocalOwnerAuthorization.rehydrate(owner, [
        device('device-1', 'credential-1'),
        device('device-1', 'credential-2'),
      ]),
    ).toThrow(new AuthorizationDomainError('AUTHORIZED_DEVICE_ID_ALREADY_AUTHORIZED'));
  });
  ...
  it('does not expose mutable owner or credential key bytes', () => {
    ...
    expect(
      authorization.recordAssertion(authorizedDeviceId('device-1'), 1, '2026-08-02T01:00:00.000Z')
        .authorizedDevices[0]?.publicKey,
  ...
  it('requires monotonic assertion counters but supports zero-counter synced credentials', () => {
    ...
    expect(() =>
      single.recordAssertion(authorizedDeviceId('device-1'), 0, '2026-08-02T01:00:00.000Z'),
    ).toThrow(new AuthorizationDomainError('CREDENTIAL_COUNTER_NOT_MONOTONIC'));
    const advanced = single.recordAssertion(
      authorizedDeviceId('device-1'),
      1,
      '2026-08-02T01:00:00.000Z',
    );
    expect(advanced.authorizedDevices[0]).toMatchObject({
      counter: 1,
      lastUsedAt: '2026-08-02T01:00:00.000Z',
    });

    const synced = LocalOwnerAuthorization.bootstrap(
      owner,
      device('device-2', 'credential-2', 'multiDevice'),
    );
    expect(
      synced.recordAssertion(authorizedDeviceId('device-2'), 0, '2026-08-02T01:00:00.000Z')
        .authorizedDevices[0]?.counter,
    ).toBe(0);
  });
  ...
  it('returns explicit first-device, rename, and revoke outcomes while preserving the final device', () => {
    ...
    const renamed = second.renameDevice(
      authorizedDeviceId('device-2'),
      deviceNickname('  Tablet  '),
    );
    expect(renamed.outcome).toMatchObject({
      kind: 'deviceRenamed',
      device: { nickname: 'Tablet' },
    });
