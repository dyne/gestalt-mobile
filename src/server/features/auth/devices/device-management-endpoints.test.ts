...
const first: AuthorizedDevice = {
  id: authorizedDeviceId('device-one'),
  credentialId: webAuthnCredentialId('credential-secret-one'),
  publicKey: new Uint8Array([9, 8, 7]),
  counter: 42,
  transports: ['internal'],
  deviceType: 'singleDevice',
  backedUp: false,
  nickname: deviceNickname('Phone'),
  createdAt: '2026-08-01T00:00:00.000Z',
  version: 3,
};
const second: AuthorizedDevice = {
  ...first,
  id: authorizedDeviceId('device-two'),
  credentialId: webAuthnCredentialId('credential-secret-two'),
  nickname: deviceNickname('Tablet'),
...
function repository(overrides: Partial<AuthorizationRepository> = {}): AuthorizationRepository {
  return {
    ...
    readOwner: () => null,
    listAuthorizedDevices: () => [first, second],
    claimFirstDevice: () => 'claimed',
    authorizeDevice: () => 'authorized',
    findDeviceByCredentialId: () => null,
    findDevice: (id) => [first, second].find((device) => device.id === id) ?? null,
    renameDevice: () => 'renamed',
    advanceCounter: () => false,
    revokeDevice: () => 'revoked',
    saveCeremony: () => {},
...
async function app(repo = repository()) {
  return buildApp({
    ...
    auth: {
      ...
      identifiers: { deviceId: () => first.id, sessionId: () => authorizationSessionId('session') },
      webauthn,
      relyingParty: rp,
    },
  });
...
describe('device-management endpoints', () => {
  ...
  it('validates Unicode nickname code points and maps unknown devices to stable problems', async () => {
    ...
    const instance = await app(
      repository({
        renameDevice: (id, _version, nickname) => {
          renamed.id = id;
          renamed.nickname = nickname;
          return 'renamed';
        },
        revokeDevice: (id) =>
          id === authorizedDeviceId('unknown-credential-material') ? 'notFound' : 'revoked',
      }),
    );
    const valid = await instance.inject({
      method: 'PATCH',
      url: '/api/auth/devices/device-two',
      headers: authenticated,
      payload: { nickname: `  ${'🧭'.repeat(64)}  ` },
    });
    ...
    expect(unknown.json()).toMatchObject({
      ...
      detail: 'The requested device is not available.',
    });
    expect(unknown.body).not.toContain('unknown');
    const unknownDelete = await instance.inject({
      method: 'DELETE',
      url: '/api/auth/devices/unknown-credential-material',
      headers: authenticated,
    });
    expect(unknownDelete.statusCode).toBe(404);
    expect(unknownDelete.json()).toMatchObject({
      code: 'DEVICE_NOT_AVAILABLE',
      detail: 'The requested device is not available.',
    });
  ...
  it('rejects malformed device IDs and trimmed-empty nicknames at the endpoint boundary', async () => {
    ...
    for (const method of ['PATCH', 'DELETE'] as const) {
      const response = await instance.inject({
        method,
        url: '/api/auth/devices/%20',
        headers: authenticated,
        ...(method === 'PATCH' ? { payload: { nickname: 'Name' } } : {}),
      });
    ...
    const empty = await instance.inject({
      method: 'PATCH',
      url: '/api/auth/devices/device-one',
      headers: authenticated,
      payload: { nickname: ' \u00a0 ' },
    });
  ...
  it('requires authentication and exact origin, clears the current cookie, and protects the final device', async () => {
    ...
    expect((await instance.inject({ method: 'GET', url: '/api/auth/devices' })).statusCode).toBe(
      401,
    );
    expect(
      (
        await instance.inject({
          method: 'DELETE',
          url: '/api/auth/devices/device-one',
          headers: { cookie: authenticated.cookie, origin: 'https://attacker.example' },
        })
      ).statusCode,
    ).toBe(403);
    ...
    const refusal = await final.inject({
      method: 'DELETE',
      url: '/api/auth/devices/device-one',
      headers: authenticated,
    });
  ...
  it('shares endpoint mutations across SQLite instances, invalidates every linked session, and redacts responses and logs', async () => {
    ...
    storeOne.claimFirstDevice(
      { id: localOwnerId('local-owner'), userHandle: new Uint8Array([1]) },
      first,
    );
    ...
    storeTwo.saveSession(authorizationSessionId('live-two-raw-token'), {
      deviceId: second.id,
      expiresAt,
    });
    const logs: string[] = [];
    ...
    const instanceOne = await buildApp({
      health: {
        read: async () => ({
          status: 'ok',
          version: 'test',
    ...
    const instanceTwo = await buildApp({
      health: {
        read: async () => ({
          status: 'ok',
          version: 'test',
    ...
    const oneHeaders = {
      cookie: 'gestalt_mobile_session=live-one-raw-token',
      origin: rp.publicOrigin,
    };
    const twoHeaders = {
      cookie: 'gestalt_mobile_session=live-two-raw-token',
      origin: rp.publicOrigin,
    };
    const renamed = await instanceOne.inject({
      method: 'PATCH',
      url: '/api/auth/devices/device-one',
      headers: oneHeaders,
      payload: { nickname: 'Renamed phone' },
    });
    expect(renamed.statusCode).toBe(204);
    const visible = await instanceTwo.inject({
      method: 'GET',
      url: '/api/auth/devices',
      headers: twoHeaders,
    });
    ...
    const revoked = await instanceOne.inject({
      method: 'DELETE',
      url: '/api/auth/devices/device-one',
      headers: oneHeaders,
    });
    ...
    expect(
      storeTwo.sessionDevice(
        authorizationSessionId('live-one-raw-token'),
        clock.now().toISOString(),
      ),
    ).toBeNull();
    expect(
      storeTwo.sessionDevice(
        authorizationSessionId('linked-one-raw-token'),
        clock.now().toISOString(),
      ),
    ).toBeNull();
    expect(
      (
        await instanceTwo.inject({
          method: 'GET',
          url: '/api/auth/devices',
          headers: { cookie: 'gestalt_mobile_session=linked-one-raw-token' },
        })
      ).statusCode,
    ).toBe(401);
    expect(
      (
        await instanceTwo.inject({ method: 'GET', url: '/api/auth/devices', headers: twoHeaders })
      ).json(),
    ).toEqual({
      devices: [
        {
    ...
    for (const token of ['%', 'expired-raw-token', 'revoked-raw-token']) {
      if (token === 'expired-raw-token')
        storeTwo.saveSession(authorizationSessionId(token), {
          deviceId: second.id,
      ...
      expect(
        (
          await instanceTwo.inject({
            method: 'GET',
            url: '/api/auth/devices',
            headers: { cookie: `gestalt_mobile_session=${token}` },
          })
        ).statusCode,
      ).toBe(401);
    }
