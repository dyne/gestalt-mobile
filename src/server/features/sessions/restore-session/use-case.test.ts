...
describe('canRestore', () => {
  ...
  it('allows replacement only for a missing rollout from a permitted source state', () => {
    ...
    expect(
      canRebindMissingRollout({ threadId: 'old-thread', state: 'stopped' } as never, missing),
    ).toBe(true);
    expect(
      canRebindMissingRollout({ threadId: 'old-thread', state: 'released' } as never, missing),
    ).toBe(true);
    expect(
      canRebindMissingRollout(
        { threadId: 'old-thread', state: 'attentionRequired' } as never,
        missing,
      ),
    ).toBe(true);
    expect(
      canRebindMissingRollout({ threadId: 'old-thread', state: 'ready' } as never, missing),
    ).toBe(false);
    expect(
      canRebindMissingRollout(
  ...
  it('does not create a replacement when the response is unrelated or malformed', () => {
    ...
    ).toThrow('CODEX_ROLLOUT_REBIND_NOT_ALLOWED');
    expect(() =>
      rebindMissingRollout(
        original as never,
        new CodexJsonRpcError(-32600, 'no rollout found for thread id old-thread'),
