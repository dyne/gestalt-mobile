...
export type ReplaceSkillProfileDependencies = Pick<SkillProfileStore, 'readGlobalProfile' | 'replaceGlobalProfile'> & { profilePath(name: string): string };

export function registerReplaceSkillProfile(app: FastifyInstance, deps: ReplaceSkillProfileDependencies): void {
