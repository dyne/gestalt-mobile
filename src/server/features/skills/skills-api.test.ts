...
describe('skills REPR endpoints', () => {
  it('maps a workspace catalog to documented effective state without contents', async () => {
    ...
    expect(response.json()).toEqual({ source: 'project', errors: [{ message: 'manifest warning' }], skills: [
      { name: 'Same', description: 'a', path: '/skills/a/SKILL.md', nativeEnabled: false, effectiveEnabled: true },
      { name: 'Same', description: 'b', path: '/skills/b/SKILL.md', nativeEnabled: true, effectiveEnabled: false },
    ] });
    await app.close();
  });
  ...
  it('lists valid and corrupt profiles independently and atomically replaces a profile', async () => {
    const app = fastify();
    const profiles = new Map<string, { version: 1; name: string; skills: Array<{ name: string; path: string; enabled: boolean }> }>();
    const deps = {
      listGlobalProfileNames: async () => ['broken', 'work'],
      readGlobalProfile: async (name: string) => { if (name === 'broken') throw new Error('bad yaml'); return profiles.get(name); },
      replaceGlobalProfile: async (profile: { version: 1; name: string; skills: Array<{ name: string; path: string; enabled: boolean }> }) => { profiles.set(profile.name, profile); },
