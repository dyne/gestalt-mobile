...
export function registerDeleteSkillProfile(app: FastifyInstance, deps: DeleteSkillProfileDependencies): void {
    ...
    try {
      ...
      if (!(await deps.deleteGlobalProfile(name)))
        return reply.code(404).type('application/problem+json').send(
          problem('SKILL_PROFILE_NOT_FOUND', 404, 'The skill profile was not found.'),
        );
      return reply.code(204).send();
    } catch (error) {
      if (error instanceof SkillProfileError)
        return reply.code(400).type('application/problem+json').send(
          problem('INVALID_SKILL_PROFILE', 400, 'The skill profile could not be deleted.'),
        );
      return reply.code(500).type('application/problem+json').send(
        problem('SKILL_PROFILE_PERSISTENCE_FAILED', 500, 'The skill profile could not be deleted.', true),
      );
    }
  });
}
