...
export function registerListAvailableSkills(
  app: FastifyInstance,
  deps: ListAvailableSkillsDependencies,
): void {
  app.get('/api/skills', async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        ...
        .send(problem('INVALID_SKILL_REQUEST', 400, 'workspaceId and profile are required.'));
    try {
      ...
      if (
        ...
        (error.message === 'WORKSPACE_NOT_FOUND' || error.message === 'PROFILE_NOT_FOUND')
      )
        return reply
          .code(404)
          .type('application/problem+json')
          .send(problem(error.message, 404, 'The requested catalog entry was not found.'));
      const code =
        error instanceof SkillProfileError && error.code === 'INVALID_SKILL_PROFILE'
          ? 'INVALID_SKILL_PROFILE'
          : 'SKILL_DISCOVERY_FAILED';
      return reply
        .code(502)
        .type('application/problem+json')
        .send(problem(code, 502, 'Skill discovery could not be completed.', true));
    }
  });
}
