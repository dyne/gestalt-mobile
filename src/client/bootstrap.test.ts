...
describe('client bootstrap', () => {
  it('scrubs an enrollment fragment before mounting code that can begin the first fetch', () => {
    ...
    const mountApp = (_component: unknown, options: { props?: { enrollmentTicket?: string; initialTheme?: string } }) => {
      order.push('mount');
      expect(options.props?.enrollmentTicket).toBe('opaque-ticket');
      expect(options.props?.initialTheme).toBe('dyne-org');
      return {} as never;
    };
    mountClient(document.createElement('div'), { hash: '#enroll=opaque-ticket', pathname: '/', search: '' } as Location, history, mountApp as never);
  ...
  it('also scrubs malformed enrollment fragments before mounting', () => {
    const order: string[] = [];
    const history = { state: null, replaceState: () => order.push('scrub') } as unknown as History;
    mountClient(document.createElement('div'), { hash: '#enroll=%E0%A4%A', pathname: '/', search: '' } as Location, history, ((_component: unknown, options: { props?: { enrollmentTicket?: string } }) => {
      order.push('mount');
