...
describe('plan controller', () => {
  it('loads the selected session and represents a missing retained plan explicitly', async () => {
    ...
    const controller = createPlanController(
      { getPlan: vi.fn().mockResolvedValueOnce(plan()).mockResolvedValueOnce(null), closePlan: vi.fn() },
      (state) => states.push(state),
    );
    controller.select('one');
    await vi.waitFor(() => expect(states.at(-1)).toMatchObject({ kind: 'ready', sessionId: 'one' }));
    controller.select('two');
    await vi.waitFor(() => expect(states.at(-1)).toEqual({ kind: 'unavailable', sessionId: 'two' }));
  });
  ...
  it('accepts complete plan replacements, retains completion, and deduplicates replay', async () => {
    ...
    controller.applyEvent('one', { sequence: 5, type: 'plan.updated', payload: plan('Complete', true) });
    expect(states.at(-1)).toMatchObject({ kind: 'ready', plan: { title: 'Complete', allDone: true } });
  ...
  it('never lets a stale session response or event render after a session switch', async () => {
    ...
    const states: PlanState[] = [];
    const controller = createPlanController(
      { getPlan: vi.fn().mockReturnValueOnce(one.promise).mockReturnValueOnce(two.promise), closePlan: vi.fn() },
      (state) => states.push(state),
    );
    controller.select('one');
    controller.select('two');
    controller.applyEvent('one', { sequence: 1, type: 'plan.updated', payload: plan('Wrong session') });
    one.resolve(plan('Stale response'));
    two.resolve(plan('Current response'));
    await vi.waitFor(() => expect(states.at(-1)).toMatchObject({ kind: 'ready', sessionId: 'two' }));
    expect(states.some((state) => state.kind === 'ready' && state.plan.title !== 'Current response')).toBe(false);
  });
  ...
  it('keeps the last good view through a transient refresh failure', async () => {
    ...
    const controller = createPlanController(
      {
        getPlan: vi.fn().mockResolvedValueOnce(plan('Stable')).mockRejectedValueOnce(new Error('offline')),
        closePlan: vi.fn(),
      },
      (state) => states.push(state),
    );
    ...
    await vi.waitFor(() => expect(states.at(-1)).toMatchObject({ kind: 'error', plan: { title: 'Stable' } }));
  });
  ...
  it('keeps completion visible with explicit feedback when close fails, and accepts plan.closed', async () => {
    const states: PlanState[] = [];
    const controller = createPlanController(
      { getPlan: vi.fn().mockResolvedValue(plan('Complete', true)), closePlan: vi.fn().mockRejectedValue(new Error('busy')) },
      (state) => states.push(state),
    );
    controller.select('one');
    await vi.waitFor(() => expect(states.at(-1)?.kind).toBe('ready'));
    await expect(controller.close()).resolves.toBeNull();
    expect(states.at(-1)).toMatchObject({ kind: 'error', error: 'busy', plan: { title: 'Complete' } });
  ...
  it('keeps a plan.closed transition and requests Chat when the pending close rejects', async () => {
    ...
    const controller = createPlanController(
      { getPlan: vi.fn().mockResolvedValue(plan('Complete', true)), closePlan: vi.fn().mockReturnValue(request.promise) },
      (state) => states.push(state),
    );
    controller.select('one');
    await vi.waitFor(() => expect(states.at(-1)?.kind).toBe('ready'));
  ...
  it('keeps a newer plan.updated transition when the pending close rejects', async () => {
    ...
    await vi.waitFor(() => expect(states.at(-1)?.kind).toBe('ready'));
    const closing = controller.close();
    controller.applyEvent('one', { sequence: 8, type: 'plan.updated', payload: plan('Replacement') });
    request.reject(new Error('late failure'));
    await expect(closing).resolves.toBeNull();
    expect(states.at(-1)).toMatchObject({ kind: 'ready', sessionId: 'one', plan: { title: 'Replacement' } });
  });
  ...
  it('does not alter a newly selected session or request navigation when a pending close succeeds', async () => {
    ...
    const controller = createPlanController(
      {
        getPlan: vi.fn().mockResolvedValueOnce(plan('Complete', true)).mockResolvedValueOnce(plan('Second session')),
    ...
    request.resolve();
    await expect(closing).resolves.toBeNull();
    expect(states.at(-1)).toMatchObject({ kind: 'ready', sessionId: 'two', plan: { title: 'Second session' } });
  });
