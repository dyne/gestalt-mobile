...
export class SessionStartController {
  ...
  async start(workspaceId: string, settings: StartSessionSettings): Promise<RelaySession | null> {
    ...
    try { const session = await this.transport.start(workspaceId, settings, this.key()); return this.#current(generation) ? session : null; }
    catch (error) { if (this.#current(generation)) this.#publish({ starting: false, error: error instanceof Error ? error.message : 'Could not start session.' }); return null; }
