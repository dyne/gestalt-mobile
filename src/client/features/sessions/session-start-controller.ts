...
export type SessionStartTransport = Readonly<{
  start(workspaceId: string, settings: StartSessionSettings, key: string): Promise<RelaySession>;
}>;
...
export class SessionStartController {
  ...
  constructor(
    private readonly transport: SessionStartTransport,
    private readonly key: () => string,
    private readonly onChange: (state: SessionStartState) => void,
  ) {}
  get state(): SessionStartState {
    return this.#state;
  }
  async start(workspaceId: string, settings: StartSessionSettings): Promise<RelaySession | null> {
    ...
    try {
      ...
      if (this.#current(generation)) this.#publish({ ...this.#state, starting: false });
    }
  }
  dispose(): void {
    this.#disposed = true;
  ...
  #publish(state: SessionStartState): void {
    if (!this.#disposed) {
      this.#state = state;
      this.onChange(state);
    }
