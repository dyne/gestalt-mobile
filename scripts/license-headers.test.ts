...
describe('source license headers', () => {
  ...
  it('inserts a generated TypeScript header idempotently before an upstream notice', () => {
    const path = 'src/server/platform/codex/generated/v2/Thread.ts';
    const generated = '// Generated by upstream.\nexport type Thread = {};\n';
    const once = insertSourceHeader(path, generated);
    expect(insertSourceHeader(path, once)).toBe(once);
    expect(once.indexOf('SPDX-License-Identifier')).toBeLessThan(
      once.indexOf('Generated by upstream'),
    );
  });
